...
= struct
  ...
  let print fmt b =
    ...
    | x::_ ->
      if (C.is_bottom x) then Format.fprintf fmt "bottom" else
  ...
  let rec assume ?(pow = 5.) b =
    ...
    if 1 = Scalar.is_infty sup then (
      if -1 = Scalar.is_infty inf then (
        ...
        assert(not (is_bottom({constraints= C.negate c :: b.constraints; env; vars})));
        ...
        if pow > 30. then 
          ...
          if Scalar.cmp inf (Scalar.of_float p2) > 0 then 
            ...
            *)
            assert(not (is_bottom({constraints= c :: c1 :: b.constraints; env; vars})));
            assert(not (is_bottom({constraints= c3 :: b.constraints; env; vars})));
      ...
      let mid = if Scalar.cmp sup (Scalar.of_int 0 ) >= 0 
                then
                   ...
                   mul_scalar sup (Scalar.of_int 2) 
      in 
      let e = Linexpr1.make env in
      Linexpr1.set_coeff e v (Coeff.s_of_int (-1)) ;
     ...
     (* s = ((m + M) / 2 *)
      ...
      (* split on: 
           ...
           b- c3 && c4  ==  m <= v <= ((m + M / 2) ) 
      *)
      assert(not (is_bottom({constraints= c :: c2 :: b.constraints; env; vars})));
      assert(not (is_bottom({constraints= c3 :: c4 :: b.constraints; env; vars})));
  ...
  let fo_assign b x e = match x with
    | A_var x ->
      let env = b.env in
      let vars = b.vars in
...
*)
  let bo_assign b x e =
    ...
    | A_var x ->
      ...
      List.iter (fun c -> Lincons1.array_set a !i c; i := !i + 1) b.constraints;
      let b = Abstract1.of_lincons_array man env a in
      let b = Abstract1.substitute_texpr man b (Var.of_string x.varId) e None in
      let a = Abstract1.to_lincons_array man b in
      let cs = ref [] in
      for i=0 to (Lincons1.array_length a)-1 do
        cs := (Lincons1.array_get a i)::!cs; (*TODO: normalization *)
