...
let typ_print fmt = function A_INT -> Format.fprintf fmt "int"
...
let aUnOp_tostring = function A_UMINUS -> "-"
let aUnOp_print fmt = function A_UMINUS -> Format.fprintf fmt "-"
...
let bBinOp_prec = function A_AND -> 2 | A_OR -> 1
let negBBinOp = function A_AND -> A_OR | A_OR -> A_AND
let bBinOp_tostring = function A_AND -> "&&" | A_OR -> "||"
...
let bUnOp_tostring = function A_NOT -> "!"
let bUnOp_print fmt = function A_NOT -> Format.fprintf fmt "!"
...
let rec aExp_invertible x e =
  ...
  | A_abinary (_, (e1, _), (e2, _)) ->
      aExp_invertible x e1 || aExp_invertible x e2
  | _ -> false
...
let rec aExp_print fmt (e, _) =
  ...
  | A_aunary (o, e1) ->
      ...
      if aExp_prec (fst e1) <= aExp_prec e then
        Format.fprintf fmt "(%a)" aExp_print e1
  ...
  | A_abinary (o, e1, e2) ->
      ...
      if aExp_prec (fst e2) <= aExp_prec e then
        Format.fprintf fmt " (%a)" aExp_print e2
...
let rec negBExp (b, a) =
  ...
  | A_MAYBE -> (A_MAYBE, a)
  | A_FALSE -> (A_TRUE, a)
  | A_bunary (o, b) -> ( match o with A_NOT -> b)
...
let rec bExp_print_aux fmt e =
  ...
  | A_bunary (o, e1) ->
      ...
      if bExp_prec (fst e1) <= bExp_prec e then
        Format.fprintf fmt "(%a)" bExp_print_aux (fst e1)
  ...
  | A_bbinary (o, e1, e2) ->
      ...
      if bExp_prec (fst e2) <= bExp_prec e then
        Format.fprintf fmt " (%a)" bExp_print_aux (fst e2)
      else Format.fprintf fmt " %a" bExp_print_aux (fst e2)
  | A_rbinary (o, e1, e2) ->
      if aExp_prec (fst e1) <= bExp_prec e then
        Format.fprintf fmt "(%a) " aExp_print e1
      else Format.fprintf fmt "%a " aExp_print e1;
      Format.fprintf fmt "%a" rBinOp_print o;
      if aExp_prec (fst e2) <= bExp_prec e then
        Format.fprintf fmt " (%a)" aExp_print e2
      else Format.fprintf fmt " %a" aExp_print e2
...
let property_print fmt p =
  ...
  if StringMap.cardinal p > 0 then
    StringMap.iter (fun l e -> Format.fprintf fmt "%s -> %a\n" l bExp_print e) p
...
and block_print ind fmt b =
  ...
  | A_block (l, s, b) ->
      Format.fprintf fmt "%a %a%a" label_print l (stmt_print ind) s
        (block_print ind) b
...
let function_print fmt f =
  ...
  | None ->
      ...
      List.iter
        (fun v -> Format.fprintf fmt "%a %a; " typ_print v.varTyp var_print v)
        f.funcArgs;
      Format.fprintf fmt "):\n";
...
let prog_print fmt (_, b, fs) =
  block_print "" fmt b;
  StringMap.iter (fun _ f -> function_print fmt f) fs
