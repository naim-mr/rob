...
(* contexts *)
type ctx = { ctxName : string; ctxTyp : var option; ctxArgs : var list }
...
(* types *)
let typ_itoa ((t : Intermediate.typ), (_ : extent)) : Ast.typ =
  match t with I_INT -> A_INT

(* expressions *)
let rec exp_itoa (ctx : ctx) (* calling context *) (env : env) (* environment *)
    ...
    ((exp : Intermediate.exp), (a : extent)) :
    Ast.exp * env * statements * statements =
  ...
  try
    ...
    | I_interval ((o1 : unaryOp option), i1, (o2 : unaryOp option), i2) ->
        let aux (o : unaryOp option) (i : string) =
          match o with
          | Some I_PLUS -> int_of_string i
          | Some I_MINUS -> -int_of_string i
          | None -> int_of_string i
        in
        (A_arithmetic (A_interval (aux o1 i1, aux o2 i2), a), env, pre, post)
    | I_incr (e, ea) | I_decr (e, ea) -> (
        (* post-increment/decrement *)
        let aux (e : Intermediate.exp) =
          match e with
          | I_incr _ -> A_PLUS
          | I_decr _ -> A_MINUS
        ...
        | I_id x ->
            ...
            ( A_arithmetic (A_var v, a),
              env,
              pre,
              post @ [ (A_assign (lhs, rhs), a) ] )
    ...
    | I_call ((e, ea), ee) -> (
        ...
        | I_id x -> (
            try
              ...
              | Some v -> (A_arithmetic (A_var v, a), env, pre @ stmt, post)
            with ItoA_error _ ->
              (* non-determinisms approximates calls to unknown functions *)
              (A_arithmetic (A_RANDOM, a), env, pre, post))
        | _ -> raise (Invalid_argument "exp_itoa:I_call"))
    | I_preincr (e, ea) | I_predecr (e, ea) -> (
    ...
    | I_mul (e1, e2) | I_div (e1, e2) | I_add (e1, e2) | I_minus (e1, e2) -> (
        ...
        in
        match (e1, e2) with
        | A_arithmetic (e1, ea1), A_arithmetic (e2, ea2) -> (
            match (e1, e2) with
    ...
    | I_eq (e1, e2) -> (
        ...
        | A_arithmetic (e1, ea1), A_arithmetic (e2, ea2) -> (
            ...
            | _ ->
                ...
                let rhs =
                  (A_rbinary (A_GREATER_EQUAL, (e1, ea1), (e2, ea2)), a)
                in
                (A_boolean (A_bbinary (A_AND, lhs, rhs), a), env, pre, post))
    ...
    | I_and (e1, e2) -> (
        ...
        | A_boolean (e1, ea1), A_boolean (e2, ea2) ->
            ( A_boolean (A_bbinary (A_AND, (e1, ea1), (e2, ea2)), a),
              env,
              pre,
              post )
    ...
    | I_or (e1, e2) -> (
        ...
        | A_arithmetic (A_RANDOM, _), _ | _, A_arithmetic (A_RANDOM, _) ->
            (A_boolean (A_MAYBE, a), env, pre, post)
        | A_boolean (e1, ea1), A_boolean (e2, ea2) ->
            ( A_boolean (A_bbinary (A_OR, (e1, ea1), (e2, ea2)), a),
              env,
              pre,
              post )
    ...
    | I_assign ((e1, ea1), o, e2) -> (
        ...
        | I_id x -> (
            ...
            let e2, env, pre, post = exp_itoa ctx env pre post e2 in
            let aux (o : Intermediate.assignOp) =
              match o with
              | I_PLUS_EQUAL -> A_PLUS
            ...
            | A_arithmetic (e2, ea2) -> (
                ...
                | I_EQUAL ->
                    let ctn =
                      pre @ [ (A_assign ((A_var v, ea1), (e2, ea2)), a) ]
                    in
                    (A_stmt, env, ctn, post)
                | I_PLUS_EQUAL | I_MINUS_EQUAL | I_MULTIPLY_EQUAL
                | I_DIVIDE_EQUAL ->
...
and call_itoa ctx (* string *) env (exp, a) =
  ...
  | I_call ((exp, ea), exps) -> (
      ...
      | I_id x when x = ctx.ctxName ->
          let stmt, env =
            List.fold_left2
              (fun (astmts, aenv) avar (aexp, aea) ->
                let aexp, aenv, apre, apost =
                  exp_itoa ctx aenv astmts [] (aexp, aea)
                in
                match aexp with
                | A_arithmetic (aexp, aea) ->
                    ( apre
                      @ [ (A_assign ((A_var avar, aea), (aexp, aea)), a) ]
                      @ apost,
                      aenv )
                | _ -> raise (Invalid_argument "call_itoa: I_call (recursion)"))
              ([], env) ctx.ctxArgs exps
          in
      ...
      | I_id x -> (
          ...
          let locals =
            StringMap.fold
              (fun _ var vm -> StringMap.add var.varId var vm)
              f.funcVars env.locals
          in
...
let property_itoa ctx env (property, a) =
  ...
  | I_particular (lbl, exp) -> (
      ...
      match exp with
      | A_boolean (exp, a) ->
          StringMap.add lbl (exp, a)
            (StringMap.add "" (A_FALSE, a) StringMap.empty)
      | _ -> raise (Invalid_argument "property_itoa:I_particular"))

(**
   Converts a parsed property to it's Ast version. In constrast to 'property_itoa', this function takes the result
...
let declarator_itoa ctx (* ctx *) gs (* var StringMap.t *) ls
    (* var StringMap.t *) scope fs (* func StringMap.t *) cs
    (* int VarMap.t *) typ
  ...
  in
  let vm =
    if scope then StringMap.add x v ls (* local scope: e.locals is updated *)
    else StringMap.add x v gs (* global scope: e.globals is updated *)
  in
  let env =
    if scope then
      { globals = gs; locals = vm; funcs = fs; return = None; constants = cs }
      (* local scope: e.locals is updated *)
    else
      { globals = vm; locals = ls; funcs = fs; return = None; constants = cs }
    (* global scope: e.globals is updated *)
  in
  let stmts, env =
    match exp with
    | Some (e, ea) -> (
        let ( e (* exp *),
              env,
              pre (* Asyntax.stmt list *),
              post (* Asyntax.stmt list *) ) =
          exp_itoa ctx env [] (* Asyntax.stmt list *) []
            (* Asyntax.stmt list *) (e, ea)
        in
        match e with
        | A_arithmetic e (* aExp annotated *) ->
            ( pre @ [ (A_assign ((A_var v, xa), e), (fst xa, snd ea)) ] @ post,
              env )
        | _ -> raise (Invalid_argument "declarator_itoa"))
    | None -> ([], env)
  in
  if scope then (env.locals (* TODO: why not vm? *), env.constants, stmts)
  else (env.globals (* TODO: was vm *), env.constants, stmts)

let globalDecl_itoa ctx (* ctx *) env (* env *) scope stmts
    (* Asyntax.stmt list *)
      ((typ (* Isyntax.typ annotated *), decls (* Isyntax.declarator list *)), _)
    =
  let vmss =
    if scope then (env.locals, env.constants, stmts)
    else (env.globals, env.constants, stmts)
  in
  List.fold_left
    (fun ( avm (* var StringMap.t *),
           acs (* aExp VarMap.t *),
           astmts (* Asyntax.stmt list *) ) decl (* Isyntax.declarator *) ->
      let vm, cs, stmts =
        if scope then
          declarator_itoa ctx env.globals avm true env.funcs acs typ
            decl (* local scope: e.locals is updated *)
        else
          declarator_itoa ctx avm env.locals false env.funcs acs typ
            decl (* global scope: e.globals is updated *)
      in
      (vm, cs, List.append astmts stmts))
    vmss decls
...
(* statements *)
let rec stmt_itoa (ctx : ctx) (* calling context *) (env : env)
    (* environment *) ((stmt : Intermediate.stmt), (a : extent)) :
    statements * env =
  (* annotated statement *)
  match stmt with
  ...
  | I_if (exp, stmt1, stmt2) -> (
      ...
      | Some stmt2 -> (
          ...
          | A_arithmetic (e, ea) ->
              let lhs = (A_rbinary (A_LESS, (e, ea), (A_const 0, ea)), ea) in
              let rhs = (A_rbinary (A_GREATER, (e, ea), (A_const 0, ea)), ea) in
              ( pre @ [ (A_if ((A_bbinary (A_OR, lhs, rhs), ea), thn, els), a) ],
                lenv2 )
          | _ -> raise (Invalid_argument "stmt_itoa: I_if"))
      | None -> (
          let els = block_itoa (post @ []) in
          match exp with
          | A_boolean exp -> (pre @ [ (A_if (exp, thn, els), a) ], lenv1)
          | A_arithmetic (A_RANDOM, ea) ->
  ...
  | I_while (exp, stmt) -> (
      ...
      match exp with
      | A_boolean exp ->
          (pre @ [ (A_while (dummyId, exp, body), a) ] @ post, lenv)
      | A_arithmetic (A_RANDOM, ea) ->
  ...
  | I_for_simple (e1, e2, e3, s) -> (
      ...
      | A_stmt, A_boolean e2, A_stmt ->
          ( pre1 @ post1 @ pre2
            @ [
                ( A_while
                    (dummyId, e2, block_itoa (post2 @ s @ pre2 @ pre3 @ post3)),
                  a );
              ]
            @ post2,
            lenv )
      ...
      | A_arithmetic (A_var _, _), A_boolean e2, A_stmt ->
          ( pre1 @ post1 @ pre2
            ...
            @ post2,
            lenv )
      | A_arithmetic (A_var _, _), A_boolean e2, A_arithmetic (A_var _, _) ->
          ( pre1 @ post1 @ pre2
            @ [
                ( A_while
                    (dummyId, e2, block_itoa (post2 @ s @ pre2 @ pre3 @ post3)),
          ...
          ( pre1 @ post1 @ pre2
            @ [
                ( A_while
                    ( dummyId,
                      (A_MAYBE, ea2),
                      block_itoa (post2 @ s @ pre2 @ pre3 @ post3) ),
                  a );
              ]
            @ post2,
            lenv )
      ...
      | A_arithmetic (A_var _, _), A_arithmetic (A_RANDOM, ea2), A_stmt ->
          ( pre1 @ post1 @ pre2
            ...
            @ post2,
            lenv )
      | ( A_arithmetic (A_var _, _),
          A_arithmetic (A_RANDOM, ea2),
          A_arithmetic (A_var _, _) ) ->
          ( pre1 @ post1 @ pre2
            @ [
                ( A_while
  ...
  | I_for (decl, exp1, exp2, stmt) -> (
      (* TODO: fix scope *)
      let locals (* var StringMap.t *), constants, stmts =
        globalDecl_itoa ctx env true (* local scope *) [] decl
      in
      let lenv = { env with locals; constants } in
          ...
          ( stmts @ pre1
            @ [
                ( A_while
                    ( dummyId,
                      e1,
                      block_itoa (post1 @ stmt @ pre1 @ pre2 @ post2) ),
                  a );
              ]
            @ post1,
            lenv )
      | A_boolean e1, A_arithmetic (A_var _, _) ->
          ( stmts @ pre1
            @ [
                ( A_while
                    ( dummyId,
      ...
      | A_arithmetic (A_RANDOM, ea1), A_stmt ->
          ( stmts @ pre1
            ...
            @ post1,
            lenv )
      | A_arithmetic (A_RANDOM, ea1), A_arithmetic (A_var _, _) ->
          ( stmts @ pre1
            @ [
                ( A_while
                    ( dummyId,
  ...
  | I_return (Some exp) -> (
      ...
      | A_arithmetic (exp, ea), Some var ->
          ( pre
            @ [ (A_assign ((A_var var, ea), (exp, ea)), a); (A_return, a) ]
            @ post,
            env )
  ...
  | I_local decl (* Isyntax.globalDecl annotated *) ->
      let locals (* var StringMap.t *), constants, stmts =
        globalDecl_itoa ctx env true (* local scope *) [] decl
      in
      let env = { env with locals; constants } in
      (stmts, env)
  | I_block stmts ->
      List.fold_left
        (fun (astmts, aenv) astmt ->
          let aastmts, aaenv = stmt_itoa ctx aenv astmt in
          (astmts @ aastmts, aaenv))
        ([], env) stmts
...
let rec label_stmt s =
  ...
  match s with
  | A_if (b, s1, s2) ->
      let s1 = label_block s1 in
      A_if (b, s1, label_block s2)
  | A_while (_, b, s) ->
      let id = newId () in
      A_while (id, b, label_block s)
  | _ -> s
...
let functionDecl_itoa env
    (* env *)
      ( ( typ (* Isyntax.typ annotated option *),
          (x (* string *), xa),
          args (* (Isyntax.typ annotated * string annotated) list *),
          stmts (* (Isyntax.stmt annotated) list *) ),
        _ ) =
  try
    let f = StringMap.find x env.funcs in
    let env = { env with locals = f.funcVars; return = f.funcTyp } in
    let ctx =
      { ctxName = f.funcName; ctxTyp = f.funcTyp; ctxArgs = f.funcArgs }
    in
    let stmts, env =
      List.fold_left
        (fun (astmts, aenv) astmt ->
  ...
  with Not_found ->
    ...
    in
    let vars =
      List.fold_left (fun vars arg -> vars @ [ arg_itoa arg ]) [] args
    in
    let locals =
      List.fold_left
        (fun alocals avar -> StringMap.add avar.varName avar alocals)
        locals vars
    in
    let env = { env with locals; return } in
...
let decl_itoa env (* env *) stmts (* Asyntax.stmt list *) decl =
  ...
  | I_global decl (* Isyntax.globalDecl annotated *) ->
      ...
      let globals (* var StringMap.t *), constants, stmts =
        globalDecl_itoa ctx env false (* global scope *) stmts decl
      in
      let env = { env with globals; constants } in
      (env, stmts)
  | I_function decl (* Isyntax.functionDecl annotated *) ->
      let funcs (* fun StringMap.t *) = functionDecl_itoa env decl in
      let env = { env with funcs } in
      (env, stmts)
...
let prog_itoa ?property (decls (* Isyntax.decl list *), _) =
  let env, stmts =
    List.fold_left
      (fun (aenv (* env *), astmts (* (Asyntax.stmt annotated) list *)) adecl ->
        decl_itoa aenv astmts adecl)
      (emptyEnv, []) decls
  in
  zeroId ();
...
let ctl_prog_itoa ctl_property main (decls, _) =
  let env, stmts =
    List.fold_left
      (fun (aenv, astmts) adecl -> decl_itoa aenv astmts adecl)
      (emptyEnv, []) decls
  in
  zeroId ();
  let block = label_block (block_itoa stmts) in
