...
module Constraint : CONSTRAINT = struct
  type t = Lincons1.t
  ...
  let compareTyp t1 t2 =
    ...
    | Lincons1.SUP, _ | _, Lincons1.SUP ->
        raise (Invalid_argument "isEqTyp:SUP")
    | Lincons1.DISEQ, _ | _, Lincons1.DISEQ ->
        raise (Invalid_argument "isEqTyp:DISEQ")
    | Lincons1.EQMOD _, _ | _, Lincons1.EQMOD _ ->
        raise (Invalid_argument "isEqTyp:EQMOD")
  ...
  let compare c1 c2 =
    let rec aux l1 l2 =
      ...
      | [], [] ->
          ...
          if c = 0 then compareTyp (Lincons1.get_typ c1) (Lincons1.get_typ c2)
          else c
      | [], (x2, v2) :: l2s ->
  ...
  let negate c =
    ...
    | Lincons1.SUPEQ ->
        ...
        Lincons1.set_cst n
          (Coeff.neg (addCoeff (Lincons1.get_cst n) (Coeff.s_of_int 1)));
        n
