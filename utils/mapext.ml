...
module Make (Ord : OrderedType) : S with type key = Ord.t = struct
  ...
  (* [AM] changed to call p in the key order *)
  let filter p s = fold (fun k d a -> if p k d then add k d a else a) s Empty
  ...
  let rec cut k = function
    ...
    | Node (l1, k1, d1, r1, h1) ->
        ...
        if c < 0 then
          ...
          (l2, d2, Node (r2, k1, d1, r1, h1))
        else if c > 0 then
          let l2, d2, r2 = cut k r1 in
          (Node (l1, k1, d1, l2, h1), d2, r2)
        else (l1, Some d1, r1)
  ...
  let rec map2 f m1 m2 =
    ...
    | Node (l1, k, d1, r1, h1) -> (
        match cut k m2 with
        | l2, Some d2, r2 -> Node (map2 f l1 l2, k, f k d1 d2, map2 f r1 r2, h1)
